...
class ResNet(nn.Module):
    ...
    def perform_training(self, train_dataset, val_dataset=None, state_dict=None, verbose=False, validation_step=5, distillation=None, policy='random', transform=None):
        ...
        train_dataset = [(img, self.label_to_index[label]) for img, label in train_dataset]
        ...
        for epoch in range(self.num_epochs):
            ...
            for images, labels in loader:
                ...
                target = F.one_hot(labels, num_classes=len(self.label_to_index)).to(DEVICE, dtype=torch.float)
                    ...
                    with torch.no_grad():
                        ...
                        if distillation == 'lfc':
                            ...
                            loss1 = nn.CosineEmbeddingLoss()(
                                cur_features,
                                old_features,
                                torch.ones(images.shape[0]
                            ).to(DEVICE)) * lmbd

                            # Preserve margin
                            old_scores = F.normalize(outputs[:,list(self.learned_classes)])
                            new_scores = F.normalize(outputs[:,new_classes])
                            ...
                            if any(hard_index):
                                ...
                                loss3 = nn.MarginRankingLoss(margin=0.5)(
                                    gt_scores.view(-1, 1),
                                    max_novel_scores.view(-1, 1),
                                    torch.ones(hard_num*K).to(DEVICE)) * (1/len(self.learned_classes)
                                )
                ...
                if distillation == 'lfc':
                    ...
                    loss = loss1 * balance + loss2 * (1 - balance) + loss3
                else:
    ...
    def perform_test(self, dataset, transform=None, classifier='fc', **classifier_kwargs):
        ...
        with torch.no_grad():
            ...
            dataset = [(img, self.label_to_index[label])for img, label in dataset]
